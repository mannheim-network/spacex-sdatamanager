...
// import { typesBundleForPolkadot, crustTypes as spacexTypes } from '@crustio/type-definitions'
...
// export type MarketFileInfo = typeof spacexTypes.market.types.FileInfo;
export type MarketFileInfo = {
  file_size: string;
  spower: string;
  expired_at: string;
...
// export type Identity = typeof spacexTypes.swork.types.Identity;
...
export default class SpaceXApi {
  ...
  async storageIdentity(): Promise<Identity | null> {
    const identity = await this.api.query.storage.identities(this.chainAccount);
    if (identity.isEmpty) {
      return null;
    }
  ...
  async workReportsAll(): Promise<unknown> {
    return parseObj(await this.api.query.storage.workReports.entries());
  }
  ...
  async currentReportSlot(): Promise<number> {
    return parseObj(await this.api.query.storage.currentReportSlot());
  }
  ...
  async groupMembers(groupOwner: string): Promise<string[]> {
    try {
      const data = await this.api.query.storage.groups(groupOwner);
      if (data.isEmpty) {
        return [];
      }
