 ...
 */
async function main() {
  logger.info('starting sdatamanager');
  const config = await loadConfig(ConfigFile);
  logger.debug('sdatamanager config loaded: %o', config);
...
async function startChain(config: NormalizedConfig) {
  ...
  const chainApi = new SpaceXApi(config.chain.endPoint, config.chain.account);
  ...
  do {
    ...
    if (lastBlock >= curBlock) {
      ...
      if (diff.asSeconds() > MaxNoNewBlockDuration.asSeconds()) {
        logger.error(
          'no new block for %d seconds, quiting sdatamanager!',
          diff.asSeconds(),
        );
